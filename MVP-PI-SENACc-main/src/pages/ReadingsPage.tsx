  ...
  return (
    <div className="animate-fade-in">
      ...
      <PageHeader
        ...
        action={<Button onClick={() => navigate('/readings/new')}><Plus className="w-4 h-4" /> Nova Leitura</Button>}
      />
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {leituras.map((l) => (
            <Card key={l.id_leitura} className="hover:shadow-md transition-shadow">
              <CardBody>
                <div className="flex items-start justify-between mb-3">
                  <div className="w-10 h-14 rounded bg-amber-100 flex items-center justify-center">
                ...
                <h3 className="font-semibold text-stone-800 mb-1">{l.livro_titulo}</h3>
                <p className="text-xs text-stone-500 mb-2">{l.clube_nome} - {l.autor_nome}</p>
                ...
                <Button variant="outline" size="sm" className="w-full" onClick={() => navigate('/meetings')}>
                  <Eye className="w-3.5 h-3.5" /> Ver Encontros
                </Button>
              </CardBody>
            </Card>
          ))}
